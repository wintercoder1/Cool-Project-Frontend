/**
 * Paddle checkout for the paid tier. PARKED — not routed while PRO_ENABLED is
 * false, so nothing in the app links here and /upgrade 404s. Kept in the build
 * so it stays type-checked rather than rotting.
 *
 * Before turning PRO_ENABLED on, two things need to change here:
 *
 *  1. Drop the email input. Once Clerk is in, upgrading requires being signed
 *     in, so the address comes from the Clerk user rather than a typed box.
 *  2. Pass the Clerk user id to Paddle as `customData`, so the Paddle webhook
 *     can map the payment back to an account and set the entitlement on that
 *     user. As written there is no way to make that link — the checkout only
 *     knows an email someone typed.
 *
 * Entitlement itself is expected to live in Clerk `publicMetadata`, written
 * server-side by the Paddle webhook and read by the frontend to unlock Pro.
 */
