...
// Strip zero-width and bidirectional control characters that sneak into names
// copied from lists/spreadsheets/RTL-aware sources. The big offender here is
// U+200E (LEFT-TO-RIGHT MARK), which shows up in URLs as %E2%80%8E and makes the
// backend lookup fail because "\u200EApple" !== "Apple". Covers ZWSP/ZWNJ/ZWJ,
// LRM/RLM, the bidi embedding/override range, word joiner, and BOM.
...
const OrganizationDetailOverview = () => {
  ...
  // The topic the URL is pointing at. Derive it; never mutate organizationData here.
  // stripInvisible removes any zero-width/bidi control chars (e.g. the U+200E that
  // shows up as %E2%80%8E) so the backend lookup uses the clean name.
  const effectiveTopic = stripInvisible(
    topic ? decodeURIComponent(topic) : organizationData.topic
  );

  // True when the URL points at a different topic than what's in localStorage/state.
  // NOTE: this compares against organizationData.topic directly. The previous version
  // mutated organizationData.topic during render, which made this flip to false on the
  // second render and caused the in-flight fetch to be cancelled before it could store
  // its result. Do not reintroduce that mutation.
  const topicIsMismatch =
    !!topic && normalize(organizationData.topic) !== normalize(effectiveTopic);
  ...
  // Keyed on the URL params (effectiveCategoryData / effectiveTopic) rather than the
  // needsFetch boolean, so cleanup only ever cancels a genuinely stale request — never
  // the one we're currently waiting on.
  useEffect(() => {
    if (!topicIsMismatch) return;
    if (!effectiveCategoryData || !effectiveTopic) return;
    if (effectiveCategoryData === 'Financial Contributions') return;
  ...
  }, [effectiveCategoryData, effectiveTopic, topicIsMismatch]);
  ...
  const openFinancialContributionPageNewTab = () => {
    ...
    const encodedTopic = encodeURIComponent(stripInvisible(currentTopic));
